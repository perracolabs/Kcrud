 ...
 * checking for a UserIdPrincipal, which is commonly used with other authentication methods.
 *
...
internal fun ApplicationCall.userFromCall(): ContextUser? {
    ...
    principal<JWTPrincipal>()?.let { jwtPrincipal ->
        ContextUser.fromJwtPayload(jwtPrincipal.payload)?.let { contextUser ->
            return contextUser
        }
    }

    // If JWTPrincipal is not present, then try from a UserIdPrincipal.
    return principal<UserIdPrincipal>()?.let { userIdPrincipal ->
        ContextUser.fromUserIdPrincipal(userIdPrincipal)
    }
}
